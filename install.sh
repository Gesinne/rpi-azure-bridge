...
" 2>/dev/null)
        ...
        4)
            ...
            echo "    2) Fase 2"
            echo "    3) Fase 3"
            echo "    4) Las 3 en columnas"
            echo "    5) Un registro concreto"
            echo ""
            echo "  Enviar/Escribir:"
            echo "    6) Enviar todos por email"
            echo "    7) Escribir un registro"
            echo ""
            echo "  Mantenimiento:"
            echo "    8) Reparar memoria corrupta"
            echo ""
            echo "    0) Volver"
            echo ""
...
EOFOSCILA
                ...
                12)
                    ...
                    # Limpiar watchdog antiguo si está instalado (deshabilitado en versiones nuevas)
                    if [ -f /etc/systemd/system/gesinne-watchdog.timer ] || [ -f /usr/local/bin/gesinne-watchdog.py ]; then
                        echo "  [!] Detectado watchdog antiguo instalado — desinstalando..."
                        sudo systemctl stop gesinne-watchdog.timer 2>/dev/null
                        sudo systemctl disable gesinne-watchdog.timer 2>/dev/null
                        sudo rm -f /etc/systemd/system/gesinne-watchdog.timer
                        sudo rm -f /etc/systemd/system/gesinne-watchdog.service
                        sudo rm -f /usr/local/bin/gesinne-watchdog.py
                        sudo systemctl daemon-reload
                        echo "  [OK] Watchdog desinstalado"
                        echo ""
                    fi
                    echo ""
                    echo "  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
...
def buscar_ultimo_backup():
    """Busca el unico backup existente"""
...
def borrar_backups_antiguos():
    """Borra backups anteriores (solo se conserva el ultimo)"""
    if not os.path.exists(BACKUP_DIR):
        return
    for f in os.listdir(BACKUP_DIR):
        if f.startswith("parametros_") and f.endswith(".txt"):
...
" 2>/dev/null
        ...
        8)
            ...
            ;;
        9)
            # Cambiar SOLO el baudrate de Node-RED (flows.json) sin tocar placas
            FIRMWARE_SCRIPT=""
            for candidate in "$USER_HOME/firmware.sh" "$INSTALL_DIR/firmware.sh"; do
                if [ -f "$candidate" ]; then
                    FIRMWARE_SCRIPT="$candidate"
